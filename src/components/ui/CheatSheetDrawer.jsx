    ...
    <div className="fixed inset-0 z-50 flex justify-end font-sans">
      ...
      <div className="relative w-full max-w-md bg-[#0B0C0E] border-l border-[#222630] h-full flex flex-col z-10 shadow-2xl">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3.5 text-sm">
          {savedClips.length === 0 ? (
            <div className="text-center py-12 text-sm text-[#9CA3AF] border border-dashed border-[#222630] p-6 rounded-xl">
            ...
            savedClips.map((clip, index) => (
              <div key={index} className="p-4 border border-[#222630] bg-[#13151A] rounded-xl">
                <div className="flex items-center justify-between border-b border-[#222630] pb-2 mb-2">
                  ...
                  <button
                    ...
                    className="text-[#9CA3AF] hover:text-red-400 text-sm"
                    title="Remove clip"
                  >
                ...
                <h4 className="text-sm font-semibold text-[#F3F4F6] mb-1">{clip.title}</h4>
                <p className="text-sm text-[#9CA3AF] leading-relaxed mb-3">{clip.snippet}</p>

                <button
                  onClick={() => copyToClipboard(`${clip.title}: ${clip.snippet}`, index)}
                  className="w-full py-2 border border-[#222630] bg-[#0B0C0E] hover:border-[#E2C08D] text-sm text-[#F3F4F6] flex items-center justify-center gap-1.5 transition-colors rounded-lg font-medium"
                >
                  {copiedIdx === index ? (
                    <>
                      <Check className="w-4 h-4 text-[#E2C08D]" />
                      <span className="text-[#E2C08D]">Copied to Clipboard</span>
                    </>
                  ) : (
                    <>
                      <Copy className="w-4 h-4 text-[#E2C08D]" />
        ...
        {savedClips.length > 0 && (
          <div className="p-4 border-t border-[#222630] bg-[#13151A] flex items-center justify-between text-sm">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-[#E2C08D] text-[#0B0C0E] text-sm font-semibold hover:bg-[#E2C08D]/90 flex items-center gap-1.5 rounded-lg"
            >
