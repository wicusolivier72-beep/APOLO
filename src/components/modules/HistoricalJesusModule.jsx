      ...
      <div className="blueprint-card p-5 sm:p-6 border border-[#222630] bg-[#13151A] rounded-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          {creedTimelineSteps.map((step, idx) => (
            <div key={idx} className="border border-[#222630] bg-[#0B0C0E] p-4.5 rounded-xl flex flex-col justify-between">
              <div>
                <span className="text-sm text-[#E2C08D] font-semibold block mb-1">{step.year}</span>
                <h4 className="text-base font-semibold text-[#F3F4F6] mb-1.5">{step.title}</h4>
        ...
        <div className="mt-4 p-4 bg-[#0B0C0E] border border-[#222630] rounded-xl text-sm text-[#9CA3AF] leading-relaxed">
          ...
          {nonChristianHistorians.map((h, i) => (
            <div key={i} className="blueprint-card p-5 border border-[#222630] bg-[#13151A] rounded-xl flex flex-col justify-between">
              <div>
                ...
                <h4 className="text-lg font-semibold text-[#F3F4F6] mb-0.5">{h.historian}</h4>
                <div className="text-sm text-[#E2C08D] font-medium mb-3">{h.work}</div>

                <blockquote className="p-3.5 bg-[#0B0C0E] border-l-2 border-l-[#E2C08D] text-sm text-[#F3F4F6] italic mb-3 leading-relaxed rounded-r-lg">
                  "{h.quote}"
                </blockquote>
