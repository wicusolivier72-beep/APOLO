            ...
            <div key={art.id} className="blueprint-card p-5 border border-[#222630] bg-[#13151A] rounded-xl flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-[#6B7280]">{art.id}</span>
                </div>

                <h3 className="text-base font-semibold text-[#F3F4F6] mb-1">{art.title}</h3>
                <div className="text-xs text-[#E2C08D] font-medium mb-3">Ref: {art.biblicalCorroboration}</div>
                ...
                <p className="text-xs text-[#9CA3AF] leading-relaxed mb-3">{art.summary}</p>

                {isExpanded && (
                  <div className="mt-3 p-3 bg-[#0B0C0E] border border-[#222630] rounded-lg text-xs space-y-2">
                    <p className="text-[#9CA3AF] leading-relaxed">{art.details}</p>
                    <div className="pt-2 border-t border-[#222630]">
                      <div className="text-xs text-[#E2C08D] font-medium mb-0.5">Primary Inscription:</div>
